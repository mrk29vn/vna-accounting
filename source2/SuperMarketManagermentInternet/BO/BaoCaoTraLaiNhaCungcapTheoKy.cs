...
namespace BizLogic
{
    public class BaoCaoTraLaiNhaCungcapTheoKy
    {
        ...
        /// <returns></returns>
        public Entities.BaoCaoTraLaiNhaCungcapTheoKy[] Select(Entities.TruyenGiaTri giatri)
        {
            Entities.BaoCaoTraLaiNhaCungcapTheoKy[] arrC = null;
            try
            {
                string sql = "exec sp_BaoCaoTraLaiNhaCungcapTheoKy @TuNgay,@DenNgay";
                conn = new Connection();
                ...
                while (dr.Read())
                {
                    Entities.BaoCaoTraLaiNhaCungcapTheoKy tao = new Entities.BaoCaoTraLaiNhaCungcapTheoKy();
                    tao.MaHDTraLaiNCC = dr[0].ToString();
                    tao.Ngaytra = DateTime.Parse(dr[1].ToString());
                    ...
                    tao.TienBoiThuong = dr[8].ToString();
                    tao.ThueGTGT = dr[9].ToString();
                    tao.ThanhToanNgay = dr[10].ToString();
                    tao.Thue = dr[11].ToString();
                    arr.Add(tao);
                }
                int n = arr.Count;
                if (n == 0) { arrC = null; }
                arrC = new Entities.BaoCaoTraLaiNhaCungcapTheoKy[n];
                for (int i = 0; i < n; i++)
                {
                    arrC[i] = (Entities.BaoCaoTraLaiNhaCungcapTheoKy)arr[i];
                }
            }
            catch (Exception ex)
